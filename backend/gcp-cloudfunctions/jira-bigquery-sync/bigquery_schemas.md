...
## Issues Table
```sql
CREATE TABLE `h2shackathon.healthcare_data.issues` (
...
## Test Cases Table
```sql
CREATE TABLE `h2shackathon.healthcare_data.test_cases` (
...
## Test Results Table
```sql
CREATE TABLE `h2shackathon.healthcare_data.test_results` (
  ...
  defect_id STRING, -- JIRA defect key if created
...
```sql
CREATE TABLE `h2shackathon.healthcare_data.compliance_reports` (
...
```sql
CREATE TABLE `h2shackathon.healthcare_data.execution_matrix` (
...
## Agents Log Table
```sql
CREATE TABLE `h2shackathon.healthcare_data.agents_log` (
...
## JIRA Sync Status Table
```sql
CREATE TABLE `h2shackathon.healthcare_data.jira_sync_status` (
  sync_id STRING NOT NULL,
  entity_type STRING, -- 'requirement', 'defect', 'feature'
  entity_id STRING, -- req_id or test_result_id
  jira_key STRING,
...
ALTER TABLE `h2shackathon.healthcare_data.requirements` 
ADD PRIMARY KEY (req_id) NOT ENFORCED;

ALTER TABLE `h2shackathon.healthcare_data.test_cases` 
ADD PRIMARY KEY (test_case_id) NOT ENFORCED;

ALTER TABLE `h2shackathon.healthcare_data.test_results` 
ADD PRIMARY KEY (test_result_id) NOT ENFORCED;

-- Create views for common queries
CREATE VIEW `h2shackathon.healthcare_data.failed_tests_without_defects` AS
SELECT 
  tr.*,
  tc.test_name,
  r.jira_id as requirement_jira_id
...
JOIN `h2shackathon.healthcare_data.requirements` r ON tc.req_id = r.req_id
WHERE tr.status = 'FAILED' 
AND (tr.defect_id IS NULL OR tr.defect_id = '');

CREATE VIEW `h2shackathon.healthcare_data.compliance_summary` AS
SELECT 
  r.req_id,
  r.jira_id,
  r.title,
...
FROM `h2shackathon.healthcare_data.requirements` r
LEFT JOIN `h2shackathon.healthcare_data.test_cases` tc ON r.req_id = tc.req_id
...
GROUP BY r.req_id, r.jira_id, r.title;
```
